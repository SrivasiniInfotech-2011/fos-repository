...
namespace FOS.Repository.Implementors
{
    ...
    public class ProspectRepository(string connectionString) : IProspectRepository
    {
        ...
        public async Task<List<GlobalParameterRequest>> Get_GlobalParameterRepository(int? companyId, int? userId)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                try
                {
                    var lstLookups = await connection.QueryAsync<GlobalParameterRequest>(SqlCommandConstants.FOS_SYSAD_GET_GlobalParameterSetup, new
                    {
        ...
        //public async Task<GlobalParameterRequest> Get_GlobalParameterRepository(int? companyId, int? userId)
        //{
        //    var company = new GlobalParameterRequest();
        //    using (var connection = new SqlConnection(connectionString))
        //    {
        //        connection.Open();
        //        var cmd = connection.CreateCommand();
        //        cmd.CommandType = CommandType.StoredProcedure;
        //        cmd.CommandText = SqlCommandConstants.FOS_SYSAD_GET_GlobalParameterSetup;
        //        cmd.Parameters.Add(new SqlParameter(SqlParameterConstants.GLOBAL_PARAMETER_COMPANY_ID, companyId, SqlParameterConstants.GLOBAL_PARAMETER_USER_ID, userId));
        ...
        //            {
        //                var dr = ds.Tables[0].Rows[0];
        //                company = new GlobalParameterRequest
        //                {
        //                    companyId = Convert.ToInt64(dr[SqlColumnNames.CompanyID]),
        //                    companyCode = Convert.ToString(dr[SqlColumnNames.CompanyCode]),
        //                    companyName = Convert.ToString(dr[SqlColumnNames.CompanyName]),
